export async function onRequestPost(context) {
  try {
    ...
    if (!userId || !code) {
      return new Response(JSON.stringify({ ok: false, error: "اكتب الكود المرسل لك." }), {
        status: 400,
        headers: { "Content-Type": "application/json; charset=utf-8" }
      });
    }

    const savedCode = await context.env.OTP_STORE.get(userId);

    if (!savedCode || savedCode !== code) {
      return new Response(JSON.stringify({ ok: false, error: "الكود غير صحيح أو منتهي الصلاحية." }), {
        status: 403,
        headers: { "Content-Type": "application/json; charset=utf-8" }
      });
    }

    // حذف الكود بعد الاستخدام (يستخدم مرة وحدة فقط)
    await context.env.OTP_STORE.delete(userId);
